...
6. **AC6 — Endpoint `GET /tc/reconciliation`.** Nuevo endpoint (RBAC `contador`/`admin`) que, dado `?card=<bank_account_id>` (y opcional `?year_month=YYYY-MM`), devuelve **una fila por cartola importada de esa tarjeta**. **Forma PLANA** (decisión Valentina 2026-07-04 — supersede la forma anidada `{cartola/ledger/laudus/checks}` de abajo; utilidad>forma, el front ya la consume): `TcReconciliationRow` con todos los campos al mismo nivel — `card, year_month, tc_real_account, lump_account, currency, fx, opening, closing, closing_clp`, C1 (`c1_ok, tc_real_balance, tc_real_native`), C2 (`c2_ok, c2_prior_closing, c2_reason`), C3 (`c3_ok, c3_corrupted_count`), C4 (`pago_cartola, laudus_payment_total, laudus_payments[]`, `pago_ok`), C5 (`c5_ok, c5_residual`), `status`, `movements[]`, `sum_compras/sum_pagos/sum_cargos`. Los nombres distinguen la fuente (`closing`=cartola, `tc_real_*`=ledger, `laudus_*`=Laudus). `card` = **`bank_account_id`** (no el stem; un stem daría 404). Enumera los períodos escaneando los asientos `source=cartola-tc` de esa tarjeta en el ledger (agrupados por `period`). `UnknownBankAccount`→404, cuenta no-TC→400. El endpoint **no** postea ni muta nada (read-only).
...
- [x] [Review][Decision→RESUELTA 2026-07-04: C1 NATIVO] **C1 USD drift multi-mes** — Veredicto Valentina: el "drift" es **diferencia de cambio no realizada**, no descuadre; C1 debe comparar la deuda en la **moneda de la cartola** (nativa), o se vuelve una alarma que grita falso cada mes que se mueve el dólar (y C1 es LA alarma anti-corrupción → un falso rojo mensual la vuelve ruido). **Implementado:** `compute_tc_cuadre` reconstruye el saldo nativo dividiendo cada pata `TC:Real` por el `fx` de SU asiento (`tc_real_native`) y C1 = `abs(tc_real_native + closing) ≤ _native_tol(currency)`. Para CLP `fx=1` → nativo == CLP → **sin regresión**. Expuesto `tc_real_native` en ambos schemas; los displays (CuadrePanel + TcReconciliationPage) muestran la comparación C1 en moneda nativa cuando USD (CLP-equiv en paréntesis). Test nuevo `test_c1_usd_multimes_nativo_no_falso_rojo` (mayo fx900 + junio fx950 → verde; con la fórmula CLP vieja habría sido rojo). Supersede la Decisión #2 (comparar en CLP×fx). [tc_cuadre.py C1 block]
- [x] [Review][Decision→RESUELTA 2026-07-04: FORMA PLANA] **AC6 respuesta plana** — Veredicto Valentina: "utilidad antes que forma" — todos los datos están en la forma plana, el frontend ya la consume; reformar a anidada = retrabajo cero-valor. **Se actualiza el AC6 abajo** para documentar la forma plana + que `card` = `bank_account_id` (no stem). Cero cambio de código. Los nombres ya distinguen la fuente (`closing`=cartola, `tc_real_balance`/`tc_real_native`=ledger, `laudus_payment_total`=Laudus).
