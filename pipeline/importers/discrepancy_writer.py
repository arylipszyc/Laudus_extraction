...
def _dedup_key(batch_id, cartola_line_no, laudus_je_id) -> tuple:
    return (str(batch_id), cartola_line_no, laudus_je_id)
...
def _existing_dedup_keys(path: Path) -> set[tuple]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        keys.add(_dedup_key(entry.get("batch_id"), cart.get("line_no"), laud.get("journal_entry_id")))
    return keys
...
def append_discrepancy(discrepancy: dict, jsonl_path: str | Path) -> bool:
    ...
    key = _dedup_key(discrepancy.get("batch_id"), cart.get("line_no"), laud.get("journal_entry_id"))
