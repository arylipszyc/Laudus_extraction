"""Spike 9.5e Task 0 — pase 6: Variación F.

Cambios sobre E:
  1. Reordena: primero qué extraer en lenguaje natural (estilo web), después formato.
  2. Quita "tarjeta de crédito chilena" (Estado-26 puede no ser TC).
  3. Refuerzo: "Para CLP, amounts son SIEMPRE enteros sin decimales".

Hipótesis: la spec técnica al inicio distrae a Gemini de la extracción real.
Hallazgo pase 4: prompt minimalista web (170 bytes) → opening perfecto.
Hallazgo pase 5: spec densa al inicio → opening fail en Estado-26.

Output: _bmad-output/coordination/9-5e-prompt-spike-f-{ts}/
"""
...
PROMPT_F = """Por favor extraé de esta cartola/estado de cuenta:
...
OUT_DIR = Path("_bmad-output/coordination") / f"9-5e-prompt-spike-f-{TS}"
...
def main() -> int:
    ...
    bytes_len = len(PROMPT_F.encode("utf-8"))
    print(f"Spike 9.5e pase 6 — model=gemini-2.5-flash, thinking auto")
    print(f"Variation F: {bytes_len} bytes")
    print(f"Output: {OUT_DIR}")
    print(f"Plan:   1 variación × {len(REPR_PDFS)} PDFs × {RUNS} runs = {len(REPR_PDFS) * RUNS} calls")
    print()

    (OUT_DIR / "prompt-F.txt").write_text(PROMPT_F, encoding="utf-8")
    summary: list[dict] = []
    ...
    for pdf_name in REPR_PDFS:
        ...
        for i in range(1, RUNS + 1):
            run_dir = OUT_DIR / f"F-{pdf_name.replace('/', '_')}-run{i}"
            run_dir.mkdir(exist_ok=True)
            try:
                raw, latency, thoughts = call_gemini(PROMPT_F, pdf_bytes, "gemini-2.5-flash")
                ...
                summary.append({"variation": "F", "pdf": pdf_name, "run": i, **metrics})
                ...
                summary.append({"variation": "F", "pdf": pdf_name, "run": i,
    ...
    print(f"  F: {n_verde}/{len(summary)} verde  avg_latency={avg_lat:.1f}s  avg_bytes={avg_bytes:.0f}")
    return 0
