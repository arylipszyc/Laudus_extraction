"""Spike 9.5e Task 0 — pase 5: Variación E.

Combina wording literal del prompt web (que encontró los openings perfectos)
con estructura mínima forzada para parseo server-side.

Hallazgo del pase 4: pedir el label exacto "Monto Facturado Anterior" (singular,
en lugar de lista de alternativas) destraba el opening.

Target: ≤800 bytes (margen sobre límite AC1 ≤1500).
Output: _bmad-output/coordination/9-5e-prompt-spike-e-{ts}/
"""
...
PROMPT_E = """Extrae las transacciones de esta tarjeta de crédito chilena y devolvé EXACTAMENTE este formato:

<header JSON en una línea>
---
<CSV con delimitador `|`>

Header JSON con estos campos:
...
Reglas:
- date: YYYY-MM-DD.
- amount: número con el signo TAL CUAL aparece en el PDF, punto decimal, sin separador de miles (1234567 no 1.234.567).
...
Devolvé SOLO el formato indicado, sin markdown ni explicaciones.
"""
...
OUT_DIR = Path("_bmad-output/coordination") / f"9-5e-prompt-spike-e-{TS}"
...
def main() -> int:
    ...
    bytes_len = len(PROMPT_E.encode("utf-8"))
    print(f"Spike 9.5e pase 5 — model=gemini-2.5-flash, thinking auto")
    print(f"Variation E: {bytes_len} bytes")
    print(f"Output: {OUT_DIR}")
    print(f"Plan:   1 variación × {len(REPR_PDFS)} PDFs × {RUNS} runs = {len(REPR_PDFS) * RUNS} calls")
    print()

    (OUT_DIR / "prompt-E.txt").write_text(PROMPT_E, encoding="utf-8")
    summary: list[dict] = []
    ...
    for pdf_name in REPR_PDFS:
        ...
        for i in range(1, RUNS + 1):
            run_dir = OUT_DIR / f"E-{pdf_name.replace('/', '_')}-run{i}"
            run_dir.mkdir(exist_ok=True)
            try:
                raw, latency, thoughts = call_gemini(PROMPT_E, pdf_bytes, "gemini-2.5-flash")
                ...
                summary.append({"variation": "E", "pdf": pdf_name, "run": i, **metrics})
                ...
                summary.append({"variation": "E", "pdf": pdf_name, "run": i,
    ...
    print(f"  E: {n_verde}/{len(summary)} verde  avg_latency={avg_lat:.1f}s  avg_bytes={avg_bytes:.0f}")
    return 0
