"""Spike 9.5e Task 0 — pase 8: Variación H (último).

Usa el wording EXACTO del prompt web (170 bytes, pase 4) que probadamente
encontró opening correcto para los 3 PDFs incluyendo Santander.
Agrega SOLO la spec mínima de formato al final.

Output: _bmad-output/coordination/9-5e-prompt-spike-h-{ts}/
"""
...
PROMPT_H = """Por favor dame un CSV con las transacciones de esta tarjeta de crédito.
Dime de qué periodo es, en qué moneda está, monto facturado anterior y monto facturado del periodo.

Devolvé tu respuesta exactamente con este formato y nada más:
...
Luego una fila por transacción del período.

Reglas:
- amount: signo TAL CUAL aparece en el PDF. Para CLP, enteros sin decimales (sin punto, sin coma): "1234567" no "1.234.567".
- date: YYYY-MM-DD.
- description: texto crudo; reemplazá `|` interno por espacio.
- Incluí cuotas pre-existentes X/N con X≥1 (sufijo " (cuota X/N)", amount = valor cuota mensual con su signo).
...
OUT_DIR = Path("_bmad-output/coordination") / f"9-5e-prompt-spike-h-{TS}"
...
def main() -> int:
    ...
    bytes_len = len(PROMPT_H.encode("utf-8"))
    print(f"Spike 9.5e pase 8 — model=gemini-2.5-flash, thinking auto")
    print(f"Variation H: {bytes_len} bytes")
    print(f"Output: {OUT_DIR}")
    print(f"Plan:   1 variación × {len(REPR_PDFS)} PDFs × {RUNS} runs = {len(REPR_PDFS) * RUNS} calls")
    print()

    (OUT_DIR / "prompt-H.txt").write_text(PROMPT_H, encoding="utf-8")
    summary: list[dict] = []
    ...
    for pdf_name in REPR_PDFS:
        ...
        for i in range(1, RUNS + 1):
            run_dir = OUT_DIR / f"H-{pdf_name.replace('/', '_')}-run{i}"
            run_dir.mkdir(exist_ok=True)
            try:
                raw, latency, thoughts = call_gemini(PROMPT_H, pdf_bytes, "gemini-2.5-flash")
                ...
                summary.append({"variation": "H", "pdf": pdf_name, "run": i, **metrics})
                ...
                summary.append({"variation": "H", "pdf": pdf_name, "run": i,
    ...
    print(f"  H: {n_verde}/{len(summary)} verde  avg_latency={avg_lat:.1f}s  avg_bytes={avg_bytes:.0f}")
    return 0
