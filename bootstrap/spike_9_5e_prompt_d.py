"""Spike 9.5e Task 0 — pase 3: Variación D.

Hereda Variación C (pipe-delimiter + header row obligatorio) y agrega:
  1. Hint corto sobre cómo identificar opening_balance (labels típicos).
  2. Hint corto sobre incluir cuotas pre-existentes X/N (X≥1).

Diagnóstico que motiva D (pase 2):
- BCI: falta capturar cuotas pre-existentes (diff 110k estable).
- Santander/Estado-26: Gemini extrae opening_balance mal (lee otro número del PDF).
  Si opening fuera el correcto (=|MONTO CANCELADO|), balance cuadra exacto.

Output: _bmad-output/coordination/9-5e-prompt-spike-d-{ts}/
"""
...
OUT_DIR = Path("_bmad-output/coordination") / f"9-5e-prompt-spike-d-{TS}"
...
def main() -> int:
    ...
    bytes_len = len(PROMPT_D.encode("utf-8"))
    print(f"Spike 9.5e pase 3 — model={model}")
    print(f"Variation D: {bytes_len} bytes")
    print(f"Output: {OUT_DIR}")
    print(f"Plan:   1 variación × {len(REPR_PDFS)} PDFs × {RUNS} runs = {len(REPR_PDFS) * RUNS} calls")
    print()

    (OUT_DIR / "prompt-D.txt").write_text(PROMPT_D, encoding="utf-8")
    summary: list[dict] = []
    ...
    for pdf_name in REPR_PDFS:
        ...
        for i in range(1, RUNS + 1):
            run_dir = OUT_DIR / f"D-{pdf_name.replace('/', '_')}-run{i}"
            run_dir.mkdir(exist_ok=True)
            try:
                raw, latency = call_gemini(PROMPT_D, pdf_bytes, model)
                ...
                summary.append({"variation": "D", "pdf": pdf_name, "run": i, **metrics})
                ...
                summary.append({"variation": "D", "pdf": pdf_name, "run": i,
    ...
    print(f"  D: {n_verde}/{len(summary)} verde  avg_latency={avg_lat:.1f}s  avg_bytes={avg_bytes:.0f}")
    return 0
